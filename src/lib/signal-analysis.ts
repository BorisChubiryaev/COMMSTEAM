...
export function normalizeAnalysis(payload: AnalysisPayload): SignalAnalysis {
  const data: SignalAnalysis = {
...
async function getSourceText(signal: SignalAnalysisInput) {
  let sourceText = [
...
export async function analyzeSignalInput(signal: SignalAnalysisInput) {
    ...
    messages: [
      {
        role: 'system',
        content: [
          'Ты — аналитик команды коммуникаций (PR/Маркетинг/Внутриком) технологической компании.',
          'Твоя задача: разобрать входящую новость/сигнал и заполнить карточку для канбан-процесса,',
          'где сигнал проходит этапы: классификация → оценка → смыслы → распределение → запуск.',
          '',
          'Как понимать поля оценки (шкала 1-5, где 5 — максимум):',
          '- relevance (актуальность): насколько новость значима и своевременна для рынка/компании сейчас.',
          '- alignment (соответствие): насколько она ложится на смыслы и цели компании.',
          '- urgency (срочность): насколько быстро нужно реагировать.',
          'priority: A — взять в работу срочно; B — важно, в плановом порядке; C — низкий приоритет; Отклонено — не наш инфоповод.',
          'meanings — смысловые направления, к которым относится новость; distribution — какие команды задействовать.',
          'potential — где новость может дать эффект; risks — чувствительность, что проверить до публикации.',
          '',
          'Правила: верни ТОЛЬКО валидный JSON без markdown и пояснений.',
          'Не выдумывай факты — опирайся только на текст. Если данных мало, выбирай осторожную оценку и более низкий приоритет.',
          'Значения для полей со справочниками бери СТРОГО из разрешённых списков (дословно). Списки-поля могут быть пустыми, если ничего не подходит.',
        ].join('\n'),
      },
      {
        role: 'user',
...
export function toIncomingAnalysisData(analysis: SignalAnalysis) {
  return {
    aiSummary: analysis.aiSummary,
    aiSource: analysis.source,
    aiSignalType: analysis.signalType,
    aiRelevance: analysis.relevance,
    aiAlignment: analysis.alignment,
    aiUrgency: analysis.urgency,
    aiPotential: analysis.potential,
    aiRisks: analysis.risks,
    aiPriority: analysis.priority,
    aiMeanings: analysis.meanings,
    aiDistribution: analysis.distribution,
    aiPublicationType: analysis.publicationType,
